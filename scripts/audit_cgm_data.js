...
async function check() {
    ...
    const res = await pg.query(`
        ...
        WHERE s."projectId" = $1 AND s.name = 'CGM PTS'
    ...
    if (res.rows.length > 0) {
        ...
        const data = row.data;
        const cols = await pg.query('SELECT id, name FROM "Column" WHERE "sheetId" = (SELECT id FROM "Sheet" WHERE name = \'CGM PTS\' AND "projectId" = $1)', [projectId]);
        const colMap = {};
        ...
        console.log('No data found with Refill Due in CGM PTS');
    }
    await pg.end();
}
check();
